...
//!
//! assert_eq!(*rrule_set.get_dt_start(), Tz::from(chrono_tz::UTC).with_ymd_and_hms(2012, 2, 1,2, 30, 0).unwrap());
...
//!
//! // Between two dates
//! let after = Tz::from(chrono_tz::UTC).with_ymd_and_hms(2012, 2, 1,10, 0, 0).unwrap();
//! let before = Tz::from(chrono_tz::UTC).with_ymd_and_hms(2012, 4, 1,9, 0, 0).unwrap();
//!
